...
load_db_option_from_env (DBOption *option)
{
    ...
    env_user = g_getenv("SEAFILE_DB_USER");
    env_passwd = g_getenv("SEAFILE_DB_PASSWORD");
    env_host = g_getenv("SEAFILE_DB_HOST");
    env_port = g_getenv("SEAFILE_DB_PORT");
    env_ccnet_db = g_getenv("SEAFILE_CCNET_DB_NAME");
    env_seafile_db = g_getenv("SEAFILE_SEAFILE_DB_NAME");
    ...
    if (env_ccnet_db && g_strcmp0 (env_ccnet_db, "") != 0) {
        ...
        seaf_message ("Failed to read SEAFILE_CCNET_DB_NAME, use ccnet_db by default\n");
    }
    if (env_seafile_db && g_strcmp0 (env_seafile_db, "") != 0) {
		...
		seaf_message ("Failed to read SEAFILE_SEAFILE_DB_NAME, use seafile_db by default\n");
    }
