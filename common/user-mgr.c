...
static int check_db_table (SeafDB *db)
{
    ...
    if (seaf_db_query (db, queries->create_table_email_user) < 0) return -1;
    if (seaf_db_query (db, queries->create_table_binding) < 0) return -1;
    if (seaf_db_query (db, queries->create_table_user_role) < 0) return -1;
    if (seaf_db_query (db, queries->create_table_ldap_users) < 0) return -1;
    if (seaf_db_query (db, queries->create_table_ldap_config) < 0) return -1;
...
ccnet_user_manager_count_emailusers (CcnetUserManager *manager, const char *source)
{
    ...
    if (manager->use_ldap && g_strcmp0(source, "LDAP") == 0) {
        gint64 ret = seaf_db_get_int64 (db, queries->ldap_user_count);
        if (ret < 0)
            return -1;
        return ret;
    }
    ...
    ret = seaf_db_get_int64 (db, queries->user_count);
    if (ret < 0)
        return -1;
    return ret;
}
...
ccnet_user_manager_count_inactive_emailusers (CcnetUserManager *manager, const char *source)
{
...
#ifdef HAVE_LDAP
    if (manager->use_ldap && g_strcmp0(source, "LDAP") == 0) {
        gint64 ret = seaf_db_get_int64 (db, queries->ldap_inactive_user_count);
        if (ret < 0)
            return -1;
        return ret;
    }
    ...
    ret = seaf_db_get_int64 (db, queries->inactive_user_count);
    if (ret < 0)
        return -1;
    return ret;
}
