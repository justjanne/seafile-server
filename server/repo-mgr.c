...
SeafDB *db = mgr->seaf->db;
    ...
    if (seaf_db_query (db, sql) < 0) return -1;
    ...
    sql = "CREATE TABLE IF NOT EXISTS RepoOwner ("
          ...
          "owner_id VARCHAR(255))";
    if (seaf_db_query (db, sql) < 0) return -1;
    seaf_db_query (db, "CREATE INDEX IF NOT EXISTS RepoOwner_owner_id_idx ON RepoOwner(owner_id)");
    ...
    if (seaf_db_query (db, sql) < 0) return -1;
    seaf_db_query (db, "CREATE INDEX IF NOT EXISTS RepoGroup_repo_id_idx ON RepoGroup(repo_id)");
    seaf_db_query (db, "CREATE INDEX IF NOT EXISTS RepoGroup_user_name_idx ON RepoGroup(user_name)");
    ...
    if (seaf_db_query (db, sql) < 0) return -1;
    seaf_db_query (db, "CREATE INDEX IF NOT EXISTS RepoUserToken_token_idx ON RepoUserToken(token)");
    seaf_db_query (db, "CREATE INDEX IF NOT EXISTS RepoUserToken_email_idx ON RepoUserToken(email)");
    ...
    if (seaf_db_query (db, sql) < 0) return -1;
    seaf_db_query (db, "CREATE INDEX IF NOT EXISTS RepoTokenPeerInfo_peer_id_idx ON RepoTokenPeerInfo(peer_id)");

    /* RepoHead */
    sql = "CREATE TABLE IF NOT EXISTS RepoHead ("
          "id BIGSERIAL PRIMARY KEY, "
          "repo_id CHAR(37) UNIQUE, branch_name VARCHAR(10))";
    if (seaf_db_query (db, sql) < 0) return -1;

    /* RepoSize - Note the quoted "size" */
    sql = "CREATE TABLE IF NOT EXISTS RepoSize ("
          "id BIGSERIAL PRIMARY KEY, "
          "repo_id CHAR(37) UNIQUE, "
          "\"size\" BIGINT, "
    ...
    sql = "CREATE TABLE IF NOT EXISTS RepoHistoryLimit ("
          "id BIGSERIAL PRIMARY KEY, "
          "repo_id CHAR(37) UNIQUE, days INTEGER)";
    if (seaf_db_query (db, sql) < 0) return -1;

    /* RepoValidSince - Note the quoted "timestamp" */
    sql = "CREATE TABLE IF NOT EXISTS RepoValidSince ("
          "id BIGSERIAL PRIMARY KEY, "
          "repo_id CHAR(37) UNIQUE, \"timestamp\" BIGINT)";
    if (seaf_db_query (db, sql) < 0) return -1;
    ...
    if (seaf_db_query (db, sql) < 0) return -1;
    seaf_db_query (db, "CREATE INDEX IF NOT EXISTS VirtualRepo_origin_repo_idx ON VirtualRepo(origin_repo)");
    ...
    /* GCID */
    sql = "CREATE TABLE IF NOT EXISTS GCID ("
          "id BIGSERIAL PRIMARY KEY, "
          "repo_id CHAR(36) UNIQUE, gc_id CHAR(36))";
    if (seaf_db_query (db, sql) < 0) return -1;

    /* LastGCID */
    sql = "CREATE TABLE IF NOT EXISTS LastGCID ("
          "id BIGSERIAL PRIMARY KEY, "
          "repo_id CHAR(36), client_id VARCHAR(128), gc_id CHAR(36), "
          "UNIQUE (repo_id, client_id))";
    if (seaf_db_query (db, sql) < 0) return -1;

    /* RepoTrash - Note quoted "size" */
    sql = "CREATE TABLE IF NOT EXISTS RepoTrash ("
          "id BIGSERIAL PRIMARY KEY, "
          "repo_id CHAR(36) UNIQUE, "
          "repo_name VARCHAR(255), head_id CHAR(40), owner_id VARCHAR(255), "
          "\"size\" BIGINT, org_id INTEGER, del_time BIGINT)";
    if (seaf_db_query (db, sql) < 0) return -1;
    seaf_db_query (db, "CREATE INDEX IF NOT EXISTS RepoTrash_owner_id_idx ON RepoTrash(owner_id)");
    seaf_db_query (db, "CREATE INDEX IF NOT EXISTS RepoTrash_org_id_idx ON RepoTrash(org_id)");
    ...
    sql = "CREATE TABLE IF NOT EXISTS RepoInfo ("
          ...
          "is_encrypted INTEGER, last_modifier VARCHAR(255), status INTEGER DEFAULT 0, type VARCHAR(10))";
    if (seaf_db_query (db, sql) < 0) return -1;
    seaf_db_query (db, "CREATE INDEX IF NOT EXISTS RepoInfo_type_idx ON RepoInfo(type)");

    /* WebUploadTempFiles */
    sql = "CREATE TABLE IF NOT EXISTS WebUploadTempFiles ("
          "id BIGSERIAL PRIMARY KEY, repo_id CHAR(40) NOT NULL, "
          "file_path TEXT NOT NULL, tmp_file_path TEXT NOT NULL)";
    if (seaf_db_query (db, sql) < 0) return -1;
    seaf_db_query (db, "CREATE INDEX IF NOT EXISTS WebUploadTempFiles_repo_id_idx ON WebUploadTempFiles(repo_id)");
