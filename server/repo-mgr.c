...
seaf_repo_get_commits (SeafRepo *repo)
{
    ...
    for (ptr = branches; ptr != nullptr; ptr = ptr->next) {
        ...
        gboolean res = seaf_commit_manager_traverse_commit_tree (seaf->commit_mgr,
                                                                 ...
                                                                 FALSE);
        if (!res) {
            for (ptr = commits; ptr != nullptr; ptr = ptr->next)
...
should_ignore_file(const char *filename, void *data)
{
    ...
    if (!g_utf8_validate (filename, -1, nullptr)) {
...
scan_trash (void *data)
{
    ...
    gint64 expire_time = time(nullptr) - trash_expire_interval;
    ...
    for (iter=repo_ids; iter; iter=iter->next) {
        repo_id = iter->data;
        ret = seaf_repo_manager_del_repo_from_trash (mgr, repo_id, nullptr);
        if (ret < 0)
            break;
    }
...
init_scan_trash_timer (SeafRepoManagerPriv *priv, GKeyFile *config)
{
    ...
    priv->scan_trash_timer = ccnet_timer_new (scan_trash, nullptr,
...
seaf_repo_manager_new (SeafileSession *seaf)
{
    ...
    pthread_rwlock_init (&mgr->priv->lock, nullptr);
...
del_repo:
    ...
    seaf_db_statement_query (mgr->seaf->db,
                             "DELETE FROM RepoTokenPeerInfo i "
                             "USING RepoUserToken t "
                             "WHERE i.token = t.token AND t.repo_id = ?",
                             1, "string", repo_id);
    ...
    for (ptr = vrepos; ptr != nullptr; ptr = ptr->next)
...
create_repo_fill_size (SeafDBRow *row, void *data)
{
    ...
    *repo = seaf_repo_new (repo_id, nullptr, nullptr);
    if (!*repo)
        return false;
...
seaf_repo_manager_get_repo_ex (SeafRepoManager *manager, const gchar *id)
{
    ...
    if (has_err) {
        ret = seaf_repo_new(id, nullptr, nullptr);
                       ...
                       int version, gboolean is_encrypted, const char *last_modifier)
{
    ...
    if (update_time == 0)
        update_time = (gint64)time(nullptr);
...
create_db_tables_if_not_exist (SeafRepoManager *mgr)
{
    ...
    if (seaf_db_query (db, queries->create_table_repo) < 0) return -1;
    ...
    if (seaf_db_query (db, queries->create_table_repo_size) < 0) return -1;
    if (seaf_db_query (db, queries->create_table_repo_history_limit) < 0) return -1;
    if (seaf_db_query (db, queries->create_table_repo_valid_since) < 0) return -1;
    if (seaf_db_query (db, queries->create_table_web_ap) < 0) return -1;
    if (seaf_db_query (db, queries->create_table_virtual_repo) < 0) return -1;
    if (seaf_db_query (db, queries->create_table_garbage_repos) < 0) return -1;

    if (queries->create_table_gcid)
        if (seaf_db_query (db, queries->create_table_gcid) < 0) return -1;
    if (queries->create_table_last_gcid)
        if (seaf_db_query (db, queries->create_table_last_gcid) < 0) return -1;

    if (seaf_db_query (db, queries->create_table_repo_trash) < 0) return -1;
                                ...
                                GError **error)
{
    ...
    if (seaf_db_statement_query (mgr->seaf->db,
                                 "DELETE FROM RepoUserToken WHERE token = ?",
                                 1, "string", token) < 0) {
        g_set_error (error, SEAFILE_DOMAIN, SEAF_ERR_GENERAL, "DB error");
        return -1;
    }
...
fill_in_token_info (GList *info_list)
{
    ...
    for (ptr = info_list; ptr; ptr = ptr->next) {
        ...
        g_object_set (info, "repo_name", repo_name, nullptr);
        g_free (repo_name);
    }
}
                                                 ...
                                                 GError **error)
{
    ...
    if (db_type == SEAF_DB_TYPE_MYSQL) {
        ...
        rc = seaf_db_statement_query (mgr->seaf->db,
                ...
                "WHERE email = ? AND token IN "
                "(SELECT token FROM RepoTokenPeerInfo WHERE peer_id = ?)",
                2, "string", email, "string", peer_id);

        if (rc >= 0) {
            rc = seaf_db_statement_query (mgr->seaf->db,
                    "DELETE FROM RepoTokenPeerInfo p "
                    "USING RepoUserToken u "
                    "WHERE p.token = u.token AND u.email = ? AND p.peer_id = ?",
                    2, "string", email, "string", peer_id);
        }
                                  ...
                                  const char *email)
{
    ...
    if (seaf_db_type(db) == SEAF_DB_TYPE_PGSQL) {
        if (seaf_db_statement_query (db,
                                     ...
                                     "ON CONFLICT (repo_id) DO UPDATE SET owner_id = EXCLUDED.owner_id",
...
collect_repos_fill_size_commit (SeafDBRow *row, void *data)
{
    ...
    gboolean is_encrypted = (bool) seaf_db_row_get_column_int (row, 6);
...
seaf_repo_manager_get_repo_list (SeafRepoManager *mgr, int start, int limit, const char *order_by, int ret_virt_repo)
{
    ...
    if (start == -1 && limit == -1) {
        ...
        case SEAF_DB_TYPE_PGSQL:
            ...
            if (!ret_virt_repo)
                g_string_append (sql, "AND v.repo_id IS NULL ");

            if (g_strcmp0 (order_by, "size") == 0)
                g_string_append (sql, "ORDER BY s.\"size\" DESC, i.repo_id");
            else if (g_strcmp0 (order_by, "file_count") == 0)
                g_string_append (sql, "ORDER BY f.file_count DESC, i.repo_id");
            else
                g_string_append (sql, "ORDER BY i.update_time DESC, i.repo_id");
            break;
        case SEAF_DB_TYPE_SQLITE:
            g_string_append (sql, "SELECT i.repo_id, s.size, b.commit_id, i.name, i.update_time, "
                "i.version, i.is_encrypted, i.last_modifier, i.status, i.type, f.file_count FROM "
        ...
        case SEAF_DB_TYPE_PGSQL:
            ...
            if (g_strcmp0 (order_by, "size") == 0)
                g_string_append (sql, "ORDER BY s.\"size\" DESC, i.repo_id LIMIT ? OFFSET ?");
            else if (g_strcmp0 (order_by, "file_count") == 0)
                g_string_append (sql, "ORDER BY f.file_count DESC, i.repo_id LIMIT ? OFFSET ?");
            else
                g_string_append (sql, "ORDER BY i.update_time DESC, i.repo_id LIMIT ? OFFSET ?");
            break;
        case SEAF_DB_TYPE_SQLITE:
            g_string_append (sql, "SELECT i.repo_id, s.size, b.commit_id, i.name, i.update_time, "
                "i.version, i.is_encrypted, i.last_modifier, i.status, i.type, f.file_count FROM "
...
collect_trash_repo (SeafDBRow *row, void *data)
{
    ...
    g_object_set (trash_repo, "encrypted", commit->encrypted, nullptr);
    seaf_commit_unref (commit);
                                       ...
                                       const char *ap)
{
    ...
    if (seaf_repo_manager_query_access_property (mgr, repo_id) == nullptr) {
        rc = seaf_db_statement_query (mgr->seaf->db,
                                          ...
                                          GError **error)
{
    ...
    if (seaf_db_statement_foreach_row (mgr->seaf->db, sql, get_group_perms_cb,
        ...
        for (p = group_perms; p != nullptr; p = p->next)
            g_free (p->data);
...
get_group_repos_cb (SeafDBRow *row, void *data)
{
    ...
    gboolean is_encrypted = (bool) seaf_db_row_get_column_int (row, 12);
    ...
    if (srepo != nullptr) {
        if (vrepo_id) {
            ...
            g_object_set (srepo, "store_id", repo_id, nullptr);
        }
        if (repo_name) {
        ...
        if (type) {
            g_object_set (srepo, "repo_type", type, nullptr);
        }
...
seaf_fill_repo_obj_from_commit (GList **repos)
{
    ...
    while (p) {
        ...
        if (!repo_name || !last_modifier) {
            g_object_get (repo, "repo_id", &repo_id, "head_cmmt_id", &commit_id, nullptr);
            commit = seaf_commit_manager_get_commit_compatible (seaf->commit_mgr,
                                                                repo_id, commit_id);
            if (!commit) {
                seaf_warning ("Commit %s not found in repo %s\n", commit_id, repo_id);
                                      ...
                                      const char *permission)
{
    ...
    if (seaf_db_type(db) == SEAF_DB_TYPE_PGSQL) {
        return seaf_db_statement_query (db,
                                        ...
                                        "ON CONFLICT (repo_id) DO UPDATE SET permission = EXCLUDED.permission",
...
collect_public_repos (SeafDBRow *row, void *data)
{
    ...
    gboolean is_encrypted = (bool) seaf_db_row_get_column_int (row, 11);
                                   ...
                                   GError **error)
{
    ...
    if (passwd) {
        ...
        rc = create_repo_common (mgr, repo_id, repo_name, repo_desc, owner_email,
                                 nullptr, nullptr, -1, nullptr, error);
    if (rc < 0)
        goto bad;
                                       ...
                                       const char *decrypted_token)
{
    ...
    token->reap_time = (gint64)time(nullptr) + DECRYPTED_TOKEN_TTL;
...
{
    SeafRepo *repo = nullptr;
    SeafCommit *commit = nullptr, *parent = nullptr;
    int ret = 0;
...
retry:
    ...
    commit = seaf_commit_new (nullptr,
                              repo->id,
                              parent->root_id,
                              user,
                              EMPTY_SHA1,
...
seaf_repo_manager_add_upload_tmp_file (SeafRepoManager *mgr,
                                       ...
                                       GError **error)
{
    char *file_path_with_slash = nullptr;

    if (file_path[0] == '/') {
        file_path_with_slash = g_strdup(file_path);
    } else {
        file_path_with_slash = g_strconcat("/", file_path, nullptr);
    }
...
seaf_repo_manager_del_upload_tmp_file (SeafRepoManager *mgr,
                                       ...
                                       GError **error)
{
    char *file_path_with_slash = nullptr, *file_path_no_slash = nullptr;
                              ...
                              GError **error)
{
    ...
    for (p = groups; p != nullptr; p = p->next) {
        group = p->data;
    ...
    for (p = groups; p; p = p->next) {
        group = p->data;
        g_object_get (group, "id", &group_id, nullptr);
        g_object_get (group, "group_name", &group_name, nullptr);

        for (; q; q = q->next) {
            repo = q->data;
            g_object_get (repo, "group_id", &repo_group_id, nullptr);
            if (repo_group_id == group_id)
                g_object_set (repo, "group_name", group_name, nullptr);
            else
                break;
        }
        g_free (group_name);
                                     ...
                                     GError **error)
{
    ...
    if (vinfo) {
        r_repo_id = vinfo->origin_repo_id;
        r_path = g_strconcat (vinfo->path, path, nullptr);
    } else {
